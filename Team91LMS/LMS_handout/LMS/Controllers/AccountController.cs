...
namespace LMS.Controllers
{
  ...
  public class AccountController : CommonController
  {
    ...
    public string CreateNewUser(string fName, string lName, DateTime DOB, string SubjectAbbrev, string role)
    {
            ...
            string first;
            int compare;
            String uidToAdd = "u0000000";
            using (Team91LMSContext db = new Team91LMSContext())
            {
                var query = from p in db.Administrators
                            orderby p.UId
                            select p.UId;
                if (query.Any())
                {
                    first = query.Last();
                    //if first is greater than we replace

                        first = first.Substring(1, 7);
                        compare = int.Parse(first);
                        if (compare > uid)
                        {
                            uid = compare;
                        }
                ...
                uid++;
                uidToAdd = "" + uid;
                for (int i = uidToAdd.Length; i < 7; i++)
                    {
                    uidToAdd = "0" + uidToAdd;
                    }
                uidToAdd = "u" + uidToAdd;
